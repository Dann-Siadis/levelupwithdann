  ...
  function handleAnswer(idx: number) {
    ...
    if (question.answers[idx].correct) {
      if (currentQ + 1 >= total) {
        ...
        setTimeout(() => { setCurrentQ(q => q + 1); setSelected(null) }, 500)
      }
    } else {
      // wrong — flash red then auto-reset
    ...
    <div className="px-5">
      ...
      <div style={{ perspective: '1200px' }}>
        ...
        >
          ...
          >
            ...
            <div className="px-5 pt-5 pb-3 border-b border-white/5">
              <div className="flex items-center gap-2">
                ...
                <img src="/quizmascot.png" alt="" width={24} height={24} style={{ width: 24, height: 24, objectFit: 'contain' }} />
              ...
              <div className="flex gap-1.5 px-5 pt-3">
                {questions.map((_, i) => (
                  <div
                    ...
                    style={{
                      background: i < currentQ ? '#e53935' : i === currentQ ? 'rgba(229,57,53,0.5)' : 'rgba(255,255,255,0.1)',
                    }}
                  />
                ))}
              </div>
            ...
            <div className="flex-1 flex flex-col justify-center px-5 py-4 gap-4">
              ...
              <div className="flex flex-col gap-2.5">
                {question?.answers.map((ans, idx) => {
                  ...
                  return (
                    ...
                    >
                      <span className="font-bold mr-2 opacity-40">{String.fromCharCode(65 + idx)}.</span>
                      {ans.text}
                    </button>
                  )
                })}
              ...
              {selected !== null && isCorrect && currentQ + 1 < total && (
                <p className="text-xs text-[#22c55e] font-semibold text-center">Correct! Next question...</p>
              )}
              {selected !== null && isCorrect && currentQ + 1 >= total && (
                <p className="text-xs text-[#22c55e] font-semibold text-center">All correct! 🎉</p>
              )}
              {selected !== null && !isCorrect && (
                <p className="text-xs text-[#ef4444] font-semibold text-center">Wrong — try again!</p>
              )}
            </div>
          ...
          >
            {flipImageUrl ? (
              <Image src={flipImageUrl} alt={flipTitle ?? 'Deal'} fill style={{ objectFit: 'cover' }} />
            ) : (
            ...
            <div className="absolute inset-0 flex flex-col justify-end p-5 gap-2.5">
              ...
              {flipTitle && <h3 className="text-lg font-extrabold text-white leading-snug">{flipTitle}</h3>}
              {flipSubtext && <p className="text-xs text-white/60 leading-relaxed">{flipSubtext}</p>}
              {flipCtaText && flipCtaLink && (
                <a
                  href={flipCtaLink}
                  target="_blank"
                  rel="noopener noreferrer sponsored"
                  className="inline-flex items-center justify-center gap-2 bg-[#e53935] hover:bg-[#c62828] text-white text-sm font-bold py-3 rounded-xl transition shadow-lg mt-1"
                >
                  {flipCtaText}
              ...
              <button
                onClick={handleReset}
                className="text-xs text-white/25 hover:text-white/50 transition text-center mt-1"
              >
                Play again
              </button>
            </div>
